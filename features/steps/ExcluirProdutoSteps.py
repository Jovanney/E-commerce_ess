...
@then('Eu clico no botão "Excluir"')
def step_impl(context):
    # Você precisa especificar o XPath correto para o botão "Carrinho"
    sleep(3)
    ...
    carrinho_button.click()
    sleep(3)
    carrinho_button = context.driver.find_element(By.XPATH, '/html/body/main/div[1]/div/div/div/div[2]/button[2]')
    carrinho_button.click()
    sleep(1)

@then('Eu vejo o modal com a mensagem "Produto deletado com sucesso"')
def step_impl(context):  # Pode precisar de um tempo para o modal aparecer
    ...
    assert modal_message.text == "Produto deletado com sucesso"
...
@then('Eu clico no botão "Comprar" do produto "Camiseta"')
def step_impl(context):
    sleep(3)
...
@then('Eu clico no botão "Comprar" do produto "rr"')
def step_impl(context):
    sleep(3)
    comprar_button = context.driver.find_element(By.XPATH, '/html/body/main/div[1]/section/div/div[2]/button')
    comprar_button.click()
    sleep(1)

@then('Eu vejo o modal com a mensagem "Produto inserido não é da mesma loja que os produtos do carrinho!"')
def step_impl(context):
    # Você pode precisar de um tempo para o modal aparecer
    modal_message = context.driver.find_element(By.XPATH, '/html/body/main/div[2]/ol/li/div/div[2]')
    assert modal_message.text == "Produto inserido não é da mesma loja que os produtos do carrinho!"